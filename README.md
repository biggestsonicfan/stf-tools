...
skips rather than failing. `test-cpres.mjs` stands the same way against a
checkout this one does not carry either: what it measures is the SHARC
disassembly — `stf-sharc`, or the `m2-hle/disassembly` it came out of — which it
finds beside this repository or is pointed at with `--disasm` or `$STF_DISASM`,
and with none to hand it skips too.

`npm test` runs every `test-*.mjs` below except `test-head-mame.mjs`, which
needs a whole display-list capture of a fight and none is carried here, so run
bare it would only ever skip. It opens with `test-romset.mjs`, which names the
explorer commit and the ROM set the rest of the run is about and refuses a set
whose members do not match their CRCs — every number after it is about that set,
so it is worth a line saying which one.

The explorer is no longer one game's. `js/games.js` now carries Sonic The
Fighters, Fighting Vipers, and three House of the Dead sets, and the decoders
take their table offsets and pointer arithmetic off it rather than baking them
in. The checks here are still Sonic The Fighters' — its captures are what there
are — but the ones that are about the board rather than about the game take a
ROM path and will read any set `js/games.js` identifies:

```sh
node test-romset.mjs ../roms/hotdp.zip
node test-decode.mjs ../roms/hotdp.zip
```
...
| `cpres.mjs` | the two SHARC coprocessor executables the program ROM carries: where they are, how long, and the one thing that separates the two forms of them. The Model 2's geometry maths runs on an ADSP-21062 beside the i960, and the SHARC has no ROM of its own — the i960 uploads its program at boot out of these two blobs. A SHARC instruction is a 48-bit PM word, and a linker writes it big end first while the ROM holds it low byte first, so `pmSwap` reverses each word and is its own inverse. Also reads a linked ADSP-21k COFF `.exe` — its section table and its `seg_pmco` bytes — which is what the disassembly shells out to `cdump` for, so a checkout without the SHARC toolchain can still run the check |
| `test-cpres.mjs` | the check, and the only one here pointed the other way: everything else measures a port against the board, this measures a *disassembly* against it. `m2-hle/disassembly` holds SHARC listings for both blobs, and the claim they make is not that they are equivalent code but that assembling and linking them produces those blobs again, byte for byte — which a ROM settles by itself. What is compared is always the linked `.exe`, never the `.obj`: the assembler leaves every absolute jump, call and dm address segment-relative, and it is `ld21k` under `sharc.ach` placing `seg_pmco` at PM 0x20000 that resolves them, so the link is part of the claim. Neither blob's address is given: the assembled image is searched for across the whole program ROM, has to turn up exactly once, and that one place has to be the offset `cpres.mjs` names — so a wrong offset and a wrong build cannot hide each other. Before that, the link has to be the one `sharc.ach` asked for, read out of `sharc.ach` rather than written down here: one `seg_pmco`, at that address, inside that window, whole 48-bit words, and as many as the ROM carries. `--build` runs the disassembly's own `build_obj.bat` first so the image was assembled a moment ago rather than whenever the `.exe` was last written, and reports which of the two were really relinked rather than taking the batch file's exit code for an answer. `--disasm` (or `$STF_DISASM`) points it at the listings; with none to hand it says so and skips, like the checks whose capture is missing |
| `test-romset.mjs` | what the rest of the suite is about to measure, run before any of it measures anything. Two facts decide whether a number here means anything, and until now neither was reported: which explorer produced it — `vendor/noclip` is pinned so a check is held against a known one, and a run that does not name the commit cannot be held against a later run — and which ROM set. `loadRomSet` CRC-checks every member it assembles against `js/games.js` and collects the failures in `rom.warnings`, and nothing read them, so a set the explorer was not written against loaded, decoded and produced numbers about a different pile of bytes. That is not hypothetical: a House of the Dead set to hand here has all twenty-four of its members mismatched, and `test-decode.mjs` reported "decoded all: 799 ok" over it without complaint. So this names the explorer commit and the game the set identifies as, fails on any CRC mismatch rather than letting fourteen checks run over it, and checks each region assembled to the size the profile declares. A region left out because the set does not carry its chips is reported and allowed — that is what `optional` is for. `--force` downgrades a mismatch to a warning, the same escape `extract-rom.mjs` gives. It takes several zip paths for a set that is split across archives, and reports a third shape of complaint beside those two: a member this set spells differently from the recipe and which was matched by its checksum instead, which is how a set labelled the way an older MAME labelled it still loads |
...
| `test-decode.mjs` | decodes every model-table entry and reports counts — the polygon decoder's smoke test. The decoder is the board's rather than one game's, so this walks whatever set it is given: the table's offset, its length and the mesh-pointer arithmetic all come off `js/games.js`, and a wrong one of the three decodes to noise rather than to nothing, which is why the triangle count is reported beside the success count. A set where almost nothing decodes fails; a sparse table does not, since an empty entry is ordinary. The named models are Sonic The Fighters' and are only printed for that set. Takes several zip paths, since a set is not always one archive — MAME splits Daytona USA's 1993 version between the clone and its parent |
...
| `test-layers.mjs` | checks the decals the recede cannot part from what they are painted on, which is why the sfight profile turns on `js/layers.js`: Flying Carpet's sand 580, whose 32 shadow triangles lie in the sand's own plane under faces too deep to recede, and Casino Night's slot machine 188, whose JACKPOT art (faces 25 and 26) stands 0.02 and 0.03 in front of faces seen square on. `coplanarLayers` has to rank every decal triangle above layer 0 and leave the rest of each model at 0, and the profile has to set `depth.layers`, without which the ranking passes but is never used. `$STF_EXPLORER` points it at another explorer checkout than `vendor/noclip` |
...
| `daytona-tables.mjs` | finds a Daytona USA build's own tables in its program ROM and prints the profile block. There is no symbol table for that title and no decompilation, so every address the explorer's profile carries has to come off an instruction — and it shipped in eight builds, which is eight chances to transcribe a number wrongly. So each routine is found by the instruction that names its table: `lda 0x1802000` is the face palette upload and states its own source, destination and length; `lda 0x12200000` beside `lda 0x12600000` is the texture bank upload, whose callers give the bank table and the shared bank; `lda 0x12800000` is the luma upload; a store at +0x10000 from `0x1800000` is the colorxlat build, whose four constants are read out of the instructions before it; a store at +0x60 from the geometry engine's base is engine function 6, the material upload. The model table is found from the other end, because nothing indexes it — it ends where the palette begins, and stepping back a stride at a time while the record is still a model record gives the base and the count, which every one of the program ROM's own pointers into that range then has to agree with. `--check` holds all of it against the profile the set loaded under and fails on any disagreement. Written against the 1993 build, whose numbers had been read by hand first, and it reproduced every one before being pointed at the other seven; it then turned up two things the hand pass would have got wrong, that the luma bands and the material table moved into the data ROM after 1993, and that the seven later builds carry a byte-identical model table. It finds the courses the same way — a course is 256 consecutive pointers into the model table and nothing else is, so the run is found first and then the array that names the runs, which is in the data ROM for every build but the 1993 one. `--rom` (repeatable), `--as <build>` to load a merged archive as one of its clones, `--check`, `--verbose`. `$STF_SITE` points it at a working explorer checkout, like `serve.mjs` |
...
| `cop-replay.mjs` | the coprocessor's current matrix, replayed from the words the i960 wrote to its FIFO: every command that writes it, as the firmware does, and the four places the stream stores a matrix and reads it back — the unit cache (`0x35`/`0x36`/`0x37`), the inner bank (`0x43`/`0x44`/`0x45`/`0x46`), the TGP slots (`0x67`, and `0x69` loading one's 3x3 turned by a facing) and the stack, with op `0x62`'s own push. The semantics are m2-hle2's port of the firmware (`sharc_exec.h`, `tools/lib/cop-replay.mjs`), held word for word against SHARC-side MAME captures there, including the two multiplies whose listing comments read the wrong way round. A matrix the stream does not carry — a bank entry never seen stored, a shadow projection, an IK solve — marks the draw `tainted`, and a check leaves it out rather than guess |
| `dl-rig.mjs` | a fighter's own matrices out of a display list, which `dl-verify.mjs`'s stage walk cannot give: a fighter is built in two passes and neither is a list of transforms on a draw. `calc_rob_angle_cont` solves the rig into TGP slots with op `0x67` (`0x3A00` for P1, `0x3B00` for P2, `0x0C` a slot) — the body after op `0x62`'s own push, the chest aimed with an ang_z and an ang_y, each limb an op `0x6B` — and works the head's aim out without storing the head. The draw pass then loads each slot's 3x3 back with op `0x69`, turned by the facing, and stores the finished parts into the unit cache with op `0x35`: body, chest and head at 0, 12 and 24. So the head is only ever whole in the draw pass, and only a full replay reaches it; this is `cop-replay.mjs` with each stored part's matrix kept on either side of its last `0x3F`, which is the part with and without its aim |
| `test-head-mame.mjs` | the chest and head against the board — the part of the rig `test-motion-mame.mjs` cannot reach, since they are matrices rather than arguments. Each is compared in the frame of the part it hangs off, so the facing and the fighter's place cancel. Asserted: the chest's own euler (object 5), the head's position on the spine and the head's own euler (object 6), against `js/pose.js` with both sides reading trig out of the coprocessor ROM. Reported and not asserted: the two aims, which are the i960's decision from the coprocessor's replies to a target it hands over (op `0x6A`) and not always the motion's — in the intros of an m2-hle2 capture the head is at times aimed about 4 units out, as far off as the other fighter stands, which a viewer with no other fighter cannot know. On all fifteen of m2-hle2's stage captures — the emulator's, which that repository grades against MAME; no MAME capture has been put through this yet — the three asserted parts agree to 6.7e-4 or better, the chest aim is the motion's on every frame, and the head aim on roughly a quarter to a half. Holds back the eight frames `smooth_int` eases a motion in over, mirrored motions and anything the replay could not follow, and counts them. Reads a `mame-motion.py` capture (P1) or an m2-hle2 `capture_dl` one (both fighters); none is carried, so it is not in `npm test` |
...
| `dl-verify.mjs` | decodes a captured display list into (model, matrix) draws — command segmentation, the matrix (`cop-replay.mjs`, carried across the frames of a capture), and the board/viewer coordinate conventions in one place. It used to apply push, pop, translate, scale and the three angles only, so a part drawn from a loaded or banked matrix came out at whatever those had left. `placeAt` places an op list under a view, cutting it at display.js's `BILLBOARD` (the coprocessor's `Fn_base_3x3`), which `boardMatrix` cannot express. Walks both ports in write order, since a part handed straight to the geometry processor is drawn at the matrix the coprocessor has built by that point |
| `verify-stage.mjs` | the check itself: every arena draw the board emitted must equal `C · M` for the viewer's own `M` and one view matrix `C` read out of the display list. Four things m2-hle2's `grade-stages.mjs` found it misreading are fixed: the backdrop's drift is taken off only on a stage whose flag bit `0x1B` says it drifts; the arena frame's vote counts the backdrop with its drift on; a frame with no view to divide by is skipped and counted rather than crashing the run (Death Egg's billboarded Earth did); and the replay is the full one, which turned up that `draw_sphynx_head` draws from inner slot 8 at 1.6 — so the 1/1.6 this used to take off the head was the old replay's doing, and the head was short by exactly the scale display.js then left off (it has since taken it on). display.js's `CAMERA_YAW`, which the capture has no angle for, is recovered from the Flying Carpet's corner posts — each a pure yaw, all one heading in a frame — and that heading then places the flames on them. Draws at a matrix the stream does not carry are counted and not compared. Prints the residual transform for any part that disagrees |
| `dl-order.mjs` | reads the board's own submission order for two models out of a display-list capture, walking both ports in write order (and runs again: it had been reading `replayFrame`'s result as the draw list itself). For the pair that needs it — South Island's floor plate (517) and the sea (555), which sort on the same four corners and so tie at every camera — the order is the only thing that decides which one is on screen, and this reads it off the board rather than guessing |
...
- *The verdict on whether any of it is right.* This is the part worth being
  ...
  `test-head-mame.mjs` asserts only what the board decides from the motion,
  and reports the two aims — which the i960 decides from state the viewer
  does not have — rather than passing or failing them.
