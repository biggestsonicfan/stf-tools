...
disassembly in `m2-hle/disassembly`, which it finds beside this repository or is
pointed at with `--disasm` or `$STF_DISASM`, and with none to hand it skips too.

`npm test` runs every `test-*.mjs` below except `test-head-mame.mjs`, which is
incomplete and says so. It opens with `test-romset.mjs`, which names the
